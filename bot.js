...
const CONFIG = {
    ...
    CHAT_ID:      'ВАШ_CHAT_ID',               // Ваш ID от @userinfobot
    FP_API_KEY:   'ВАШ_FAUCETPAY_API_KEY',     // API ключ FaucetPay
    FP_ADDRESS:   'ВАШ_ОСНОВНОЙ_АДРЕС',        // Ваш BTC адрес на FaucetPay
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'status')          sendStatus(chatId);
        ...
        bot.sendMessage(chatId, '⏹ *Автосбор остановлен*', { parse_mode: 'Markdown', ...MAIN_KEYBOARD });
...
function startAutoCollect(chatId) {
    if (autoTimer) clearInterval(autoTimer);
    bot.sendMessage(chatId, '✅ Автосбор *запущен!*\n\nПроверяю краны каждые 5 минут.\nНажимайте кнопку когда получите уведомление о готовом кране! 🚰', { parse_mode: 'Markdown', ...MAIN_KEYBOARD });
